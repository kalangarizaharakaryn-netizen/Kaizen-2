...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        speechRecognizer.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onRmsChanged(rmsdB: Float) {
                // Real mic level, straight from the recognizer — feeds the ring while listening.
        ...
        binding.wifiButton.setOnClickListener { appendLog("Kaizen", Hardware.openWifiSettings(this)) }
    ...
    /** Real audio isn't easy to read back out of TextToSpeech, so this drives a smooth
     *  sine pulse for the duration Android actually reports speech is playing — tied to
     *  real onStart/onDone events, not a length estimate. */
    ...
    private fun handleCommand(text: String) {
        ...
        // Typed hardware commands — same actions the buttons trigger, so you don't need to tap.
        Regex("^(turn off|disable) (the )?bluetooth", RegexOption.IGNORE_CASE).find(text)?.let {
            val out = Hardware.openBluetoothSettingsForOff(this)
            appendLog("Kaizen", out); speak(out); setStatus("STANDBY")
            return
        }
        Regex("^(turn on|enable|check) (the )?bluetooth", RegexOption.IGNORE_CASE).find(text)?.let {
            val out = Hardware.requestEnableBluetooth(this, enableBtLauncher)
            appendLog("Kaizen", out); speak(out); setStatus("STANDBY")
            return
        }
        Regex("^(turn on|turn off|enable|disable|check|toggle) (the )?wi-?fi", RegexOption.IGNORE_CASE).find(text)?.let {
            val out = Hardware.openWifiSettings(this)
            appendLog("Kaizen", out); speak(out); setStatus("STANDBY")
            return
        }
        Regex("^(turn on|turn off|toggle) (the )?(torch|flashlight)", RegexOption.IGNORE_CASE).find(text)?.let {
            val out = Hardware.toggleTorch(this)
            appendLog("Kaizen", out); speak(out); setStatus("STANDBY")
            return
        }
        Regex("^(add|create|schedule|set) (a |an )?(calendar )?event", RegexOption.IGNORE_CASE).find(text)?.let {
            showAddEventDialog()
            setStatus("STANDBY")
            return
        }
        Regex("^(where am i|check my location|get my location|find my location)", RegexOption.IGNORE_CASE).find(text)?.let {
            fetchLocation()
            setStatus("STANDBY")
            return
        }
